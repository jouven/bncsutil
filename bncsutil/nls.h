 ...
 */
extern nls_t* nls_init(const char* username, const char* password);
 ...
 */
extern nls_t* nls_init_l(const char* username, unsigned long username_length,
    const char* password, unsigned long password_length);

/**
 * Frees an nls_t structure.
 */
extern void nls_free(nls_t* nls);
 ...
 * pointer on failure.
 */
extern nls_t* nls_reinit(nls_t* nls, const char* username,
    const char* password);
 ...
 */
extern nls_t* nls_reinit_l(nls_t* nls, const char* username,
 ...
 */
extern unsigned long nls_account_create(nls_t* nls, char* buf, unsigned long bufSize);
 ...
 * Returns the number of bytes placed in buf, or 0 on error.
 */
extern unsigned long nls_account_logon(nls_t* nls, char* buf, unsigned long bufSize);
 ...
 */
extern nls_t* nls_account_change_proof(nls_t* nls, char* buf,
 ...
 */
extern void nls_get_S(nls_t* nls, char* out, const char* B, const char* salt);

/**
 * Gets the password verifier (v). (32 bytes)
 */
extern void nls_get_v(nls_t* nls, char* out, const char* salt);

/**
 * Gets the public key (A). (32 bytes)
 */
extern void nls_get_A(nls_t* nls, char* out);

/**
 * Gets "K" value, which is based on the secret (S).
 * The buffer "out" must be at least 40 bytes long.
 */
extern void nls_get_K(nls_t* nls, char* out, const char* S);

/**
 * Gets the "M[1]" value, which proves that you know your password.
 * The buffer "out" must be at least 20 bytes long.
 */
extern void nls_get_M1(nls_t* nls, char* out, const char* B, const char* salt);
 ...
 * safely set to NULL.
 */
extern int nls_check_M2(nls_t* nls, const char* var_M2, const char* B,
    const char* salt);
 ...
 */
extern int nls_check_signature(uint32_t address, const char* signature_raw);
