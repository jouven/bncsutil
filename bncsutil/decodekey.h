 ...
 */
extern int kd_quick(const char* cd_key, uint32_t client_token,
 ...
 * Returns the length or 0 on failure.
 */
extern int kd_val2(int decoder);
 ...
 * Returns the length or 0 on failure.
 */
extern int kd_longVal2(int decoder, char* out);

/**
 * Calculates the hash of the key values for SID_AUTH_CHECK (0x51).
 * Returns the hash length or 0 on failure.
 */
extern int kd_calculateHash(int decoder, uint32_t clientToken,
 ...
 * Returns the hash length or 0 on failure.
 */
extern int kd_getHash(int decoder, char* out);
 ...
 *     if it was invalid.
 */
extern int kd_isValid(int decoder);
