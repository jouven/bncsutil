...
nls_t* nls_init(const char* username, const char* password) {
    return nls_init_l(username, (unsigned long) strlen(username),
        password, (unsigned long) strlen(password));
}

nls_t* nls_init_l(const char* username, unsigned long username_length,
...
nls_t* nls_reinit(nls_t* nls, const char* username,
    const char* password)
{
...
nls_t* nls_reinit_l(nls_t* nls, const char* username,
...
unsigned long nls_account_create(nls_t* nls, char* buf, unsigned long bufSize) {
    mpz_t s; /* salt */
    mpz_t v;
    mpz_t x;
...
unsigned long nls_account_logon(nls_t* nls, char* buf, unsigned long bufSize) {
    if (!nls)
        return 0;
...
nls_t* nls_account_change_proof(nls_t* nls, char* buf,
...
void nls_get_S(nls_t* nls, char* out, const char* B, const char* salt) {
    mpz_t temp;
    mpz_t S_base, S_exp;
    mpz_t x;
    mpz_t v;
...
void nls_get_v(nls_t* nls, char* out, const char* salt) {
    mpz_t g;
    mpz_t v;
    mpz_t x;
...
void nls_get_A(nls_t* nls, char* out) {
    mpz_t g;
    mpz_t A;
    size_t o;
...
void nls_get_K(nls_t* nls, char* out, const char* S) {
    char odd[16], even[16];
...
void nls_get_M1(nls_t* nls, char* out, const char* B, const char* salt) {
    SHA1Context sha;
...
int nls_check_M2(nls_t* nls, const char* var_M2, const char* B,
    const char* salt)
{
    SHA1Context sha;
    char local_M2[20];
...
int nls_check_signature(uint32_t address, const char* signature_raw) {
    char* result_raw;
    char check[32];
    mpz_t result;
    mpz_t modulus;
